...
def print_table(players, label=None):
    if label:
        print(f"\n{label}")
    print(f"  {'Player':<22} {'Rating':>7}  {'Avg HC (30g)':>12}  {'Rec. HC':>7}  {'Games':>5}  {'Confidence':>10}")
    print(f"  {'-'*72}")
    for r in players:
        ...
        rec_str = f"{rec}%"
        print(f"  {r['name']:<22} {r['mu_scaled']:>7.0f}  {avg_str:>12}  {rec_str:>7}  {r['games_played']:>5}  {r['confidence_percent']:>9.1f}%")


def main():
    ratings_path = os.path.join(PROJECT_ROOT, 'player_ratings.json')
    if not os.path.exists(ratings_path):
        print(f"Error: {ratings_path} not found. Run calculate_trueskill.py first.")
        sys.exit(1)
    ...
    provisional = [r for r in ratings if r['games_played'] < config.MIN_GAMES_FOR_RANKING]
    ...
    if provisional:
        print_table(provisional, label=f"--- Provisional (< {config.MIN_GAMES_FOR_RANKING} games) ---")
